...
fun StockChart(
    ...
    val spacing = 100f
    val trasnparentGraphColor = remember {
    ...
    Canvas(modifier = modifier) {
        //calculation for spacing for x axis (horizontal)
        val spacePerHour = (size.width - spacing) / infos.size
        Log.d(TAG, "spacePerHour: $spacePerHour")
        (0 until infos.size - 1 step 2).forEach { i ->
            val info = infos[i]
            val hour = info.date.hour
            //this will allow us to write a text in canvas
